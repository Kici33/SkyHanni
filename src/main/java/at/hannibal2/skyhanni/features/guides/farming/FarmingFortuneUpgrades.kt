...
object FarmingFortuneUpgrades {
    ...
    val genericUpgrades = mutableListOf<FarmingFortuneUpgrade>()
    val cropSpecificUpgrades = mutableListOf<FarmingFortuneUpgrade>()
    ...
    private fun MutableList<FarmingFortuneUpgrade>.populateAndSort(style: Int) {
        this.map { upgrade ->
    ...
    private fun getEquipmentUpgrades() {
        ...
        for (piece in FarmingItems.equip) {
            ...
            when (item.getReforgeName()) {
                "rooted" -> {}
                "blooming" -> {
                    ReforgeAPI.reforgeList.find { it.name == "BLOOMING" }?.let { reforge ->
                        reforgeItem(item, ReforgeAPI.reforgeList.firstOrNull { it.name == "Rooted" }!!, cropSpecificUpgrades );
                    }
                }

                else -> {
                    reforgeItem(item, ReforgeAPI.reforgeList.firstOrNull { it.name == "Blooming" }!!, cropSpecificUpgrades );
                }
            }
        }
    }
    ...
    private fun getArmorUpgrades() {
        for (piece in FarmingItems.armor) {
            ...
            when (item.getReforgeName()) {
                "mossy" -> {}
                "bustling" -> {
                    reforgeItem(item, ReforgeAPI.reforgeList.firstOrNull { it.name == "Mossy" }!!, cropSpecificUpgrades );
                }
                else -> {
                    reforgeItem(item, ReforgeAPI.reforgeList.firstOrNull { it.name == "Bustling" }!!, cropSpecificUpgrades );
                }
            }
        }
    }
    ...
    fun getCropSpecific(tool: ItemStack?) {
        ...
        if (cultivatingLvl == 0) {
            cropSpecificUpgrades.add(
                FarmingFortuneUpgrade("§7Enchant your ${tool.displayName} §7with Cultivating", null, "CULTIVATING;1", 1, 12.0)
            )
        }
        if (turboCropLvl != 5) {
            cropSpecificUpgrades.add(
        ...
        when (tool.getReforgeName()) {
            ...
            "bountiful" -> {}
                ...
                reforgeItem(tool, ReforgeAPI.reforgeList.firstOrNull { it.name == "Blessed" }!!, cropSpecificUpgrades );
            }
        }
        cropSpecificUpgrades.populateAndSort(0)
    }

    private fun recombobulateItem(item: ItemStack, list: MutableList<FarmingFortuneUpgrade>) {
        if (item.isRecombobulated()) return

        val reforge = item.getReforgeName().let { reforgeName ->
            ReforgeAPI.reforgeList.firstOrNull { it.name == reforgeName }
        } ?: return

        FarmingFortuneDisplay.loadFortuneLineData(item, 0.0)
        val increase = reforge.stats[LorenzRarity.getById(item.getItemRarityOrCommon().id + 1)!!]?.get(SkyblockStat.FARMING_FORTUNE) ?: return
        list.add(
            FarmingFortuneUpgrade("§7Recombobulate your ${item.displayName}", null, "RECOMBOBULATOR_3000", 1, increase)
        )
    }
    ...
    private fun reforgeItem(
        ...
        list: MutableList<FarmingFortuneUpgrade>,
        copperPrice: Int? = null,
    ) {
        FarmingFortuneDisplay.loadFortuneLineData(item, 0.0)
        val increase = reforge.stats[item.getItemRarityOrCommon()] ?: return
        list.add(
            FarmingFortuneUpgrade(
                "§7Reforge your ${item.displayName} §7to ${reforge.name}",
                copperPrice, reforge.reforgeStone!!.itemName, 1, increase[SkyblockStat.FARMING_FORTUNE] ?: 0.0
            )
        )
    }
